...
if __name__ == "__main__":
    ...
    test_map = ImageMapping(left_Kl, left_Kr, left_R, left_T, size_dst, size_src, 1)
    test_map_gray = ImageMapping(gray_Kl, gray_Kr, gray_R, gray_T, size_dst, size_dst, 1)
    # test_map_right = ImageMapping(right_Kl, right_Kr, right_R, right_T, size_src, size_dst, 1)
    while not running:
        t1 = MyThread(cam2)
        t1.start()
        t1.join()
        ...
        if not save_calib_image:
            ...
            # 图像融合
            region, depth = get_region_depth(frame_1, frame_l, test_map)
            out = test_map.fusion(frame_1, frame_l, region, depth[0])
            cv2.imshow("out", out)
            cv2.waitKey(1)

            # region_rrr, depth_rrr = get_region_depth(frame_2, frame_1, test_map_gray)
            # out_right = test_map_gray.fusion(frame_2, frame_1, region_rrr, depth_rrr[0])
            # cv2.imshow("out_right", out_right)
            # cv2.waitKey(1)

            # region_lll, depth_lll = get_region_depth(frame_r, frame_2, test_map_right, 1)
            # out_lll = test_map_right.fusion(frame_r, frame_2, region_lll, depth_lll[0], 1)
            # cv2.imshow("out_lll", out_lll)
            # cv2.waitKey(1)
