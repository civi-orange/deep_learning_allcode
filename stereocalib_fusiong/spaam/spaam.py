...
class SPAAM():
    ...
    def get_camera_matrix(self):
        ...
        CC = np.zeros((2 * self.numSamples, 12))
        ...
        for i in range(self.numSamples):
            ...
            image1 = self.pImage[i]
            world1 = self.pWorld[i]
            ...
            CC[i * 2][6] = -world1[2]
            CC[i * 2][7] = -1
            CC[i * 2][8] = image1[1] * world1[0]
            CC[i * 2][9] = image1[1] * world1[1]
            CC[i * 2][10] = image1[1] * world1[2]
            CC[i * 2][11] = image1[1]
            CC[i * 2 + 1][0] = world1[0]
            CC[i * 2 + 1][1] = world1[1]
            CC[i * 2 + 1][2] = world1[2]
            ...
            CC[i * 2 + 1][6] = 0
            CC[i * 2 + 1][7] = 0
            CC[i * 2 + 1][8] = -image1[0] * world1[0]
            CC[i * 2 + 1][9] = -image1[0] * world1[1]
            CC[i * 2 + 1][10] = -image1[0] * world1[2]
            CC[i * 2 + 1][11] = -image1[0]

        _, _, TEST = np.linalg.svd(CC)
        TEST = TEST.transpose()
        testssssss = TEST[:, 11].reshape(3, 4)
        ...
        isNegative = self.G[2, 0] * self.pWorld[1, 0] + self.G[2, 1] * self.pWorld[0, 1] + self.G[2, 2] * \
                     self.pWorld[0, 2] + self.G[2, 3]
        if isNegative < 0:
            self.G *= -1

        return self.G, testssssss
