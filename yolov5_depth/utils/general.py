...
def labels_to_class_weights(labels, nc=80):
    ...
    labels = np.concatenate(labels, 0)  # labels.shape = (866643, 5) for COCO  ---depth has 6 members
    classes = labels[:, 0].astype(np.int)  # labels = [class xywh] --depth
...
def segments2boxes(segments):
    # Convert segment labels to box labels, i.e. (cls, xy1, xy2, ...) to (cls, xywh) ---depth
    boxes = []
    for s in segments:
        x, y = s.T  # segment xy
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    """
    # 所有的预测列表 减去x y w h iou_conf 和 其他 留下分类
    ...
    t = time.time()
    # x y w h iou_conf*cls_conf cls depth
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        conf = x[:, 4:-1]
        depth = x[:, -1]  # x[:, 7]
        # Detections matrix nx6 (xyxy, conf, cls)   add depth
        if multi_label:
